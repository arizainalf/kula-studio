...
staff.post('/invite', requireRole('manager', 'admin_studio', 'platform_admin'), async (c) => {
  ...
  const [newUser] = await sql`
    insert into users (email, password_hash, name, role, is_active, plan_tier, expires_at, studio_id, avatar_url, youtube_url, phone)
    values (${parsed.data.email}, ${hashPassword(parsed.data.password)}, ${parsed.data.name},
            ${assignedRole}, true, ${parsed.data.plan_tier}, ${parsed.data.expires_at}, ${assignedStudioId ?? null}, ${parsed.data.avatar_url ?? null}, ${parsed.data.youtube_url ?? null}, ${parsed.data.phone ?? null})
    returning id, email, name, role, plan_tier, expires_at, is_active, studio_id, avatar_url, youtube_url, phone, created_at`;
